...
> A Claude Code skill that finds, vets, and installs **other** skills — ranked by
> **real per-skill install counts**, screened by a three-tier security scan, and
> installed only after you explicitly confirm.
>
...
**方式一：直接 clone 进 skills 目录**（仓库名已与 skill 名一致）

```bash
cd ~/.claude/skills
git clone https://github.com/<owner>/find-the-skill-you-want-most.git
```

**方式二：走 `npx skills`**

```bash
npx skills add <owner>/find-the-skill-you-want-most
```

装完**重启 Claude Code**，即可作为 `/find-the-skill-you-want-most` 调用，
或直接说"帮我找个能做 X 的 skill"。

**要求**：Node.js ≥ 20（开发与验证在 v24.14.1 上完成）。无需 token，无需 `jq`。
...
> 表格里**没有 stars 列**，也不展开各维度权重。权重只写在 [`SCORING.md`](SCORING.md) 供审计，
> 表格只呈现可直接核对的硬事实。
